...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 DRIP at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zDRIP send
enum ZerocoinSpendStatus {
    ZDRIP_SPEND_OKAY = 0,                            // No error
    ZDRIP_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDRIP_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDRIP_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDRIP_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDRIP_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDRIP_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDRIP_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDRIP_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDRIP_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDRIP_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDRIP_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDRIP_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDRIP_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDRIP_SPENT_USED_ZDRIP = 14,                      // Coin has already been spend
    ZDRIP_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZDRIP_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZDripBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZDRIPOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzdripAuto) const;
    ...
    */
    ...
    CzDRIPWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzDRIPTracker> zdripTracker;
    ...
    void setZWallet(CzDRIPWallet* zwallet)
    {
        zwalletMain = zwallet;
        zdripTracker = std::unique_ptr<CzDRIPTracker>(new CzDRIPTracker(strWalletFile));
    }

    CzDRIPWallet* getZWallet() { return zwalletMain; }
    ...
    void setZDripAutoBackups(bool fEnabled)
    {
    ...
    /** zDRIP reset */
    boost::signals2::signal<void()> NotifyzDRIPReset;
