...
CzDRIPTracker::CzDRIPTracker(std::string strWalletFile)
{
...
CzDRIPTracker::~CzDRIPTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDRIPTracker::Init()
{
...
bool CzDRIPTracker::Archive(CMintMeta& meta)
{
...
bool CzDRIPTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDRIPTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDRIPTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDRIPTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzDRIPTracker::GetSerialHashes()
{
...
CAmount CzDRIPTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDRIPTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDRIPTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDRIPTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDRIPTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDRIPTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDRIPTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDRIPTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDRIPTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDRIPTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDRIPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDRIPTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDRIPTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDRIPTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDRIPTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDRIPTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDRIPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzdrip from DB\n", __func__, listDeterministicDB.size());
    }
