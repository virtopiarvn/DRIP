...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "drip.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "dripd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the DRIP and zDRIP money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 21461, 21463));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in DRIP/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in DRIP/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, drip, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in DRIP/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all DRIP specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-dripstake=<n>", strprintf(_("Enable or disable staking functionality for DRIP inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zdripstake=<n>", strprintf(_("Enable or disable staking functionality for zDRIP inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:21461"));
    ...
    strUsage += HelpMessageOpt("-backupzdrip=<n>", strprintf(_("Enable automatic wallet backups triggered after each zDRIP minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zdripbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zDRIP backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizedripamount=<n>     " + strprintf(_("Keep N DRIP anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 21462, 21464));
...
/** Sanity checks
 *  Ensure that DRIP is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. DRIP Core is shutting down."));
...
#endif
    // Make sure only a single DRIP process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DRIP Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //DRIP: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //DRIP specific: zerocoin and spork DB's
                ...
                // DRIP: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZDRIPMinted();
                        RecalculateZDRIPSpent();
                    }
                    RecalculateDRIPSupply(1);
                }
                ...
                // DRIP: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of DRIP Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart DRIP Core to complete") << "\n";
        ...
        zwalletMain = new CzDRIPWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zDRIPWallet
        uiInterface.InitMessage(_("Syncing zDRIP wallet..."));

        bool fEnableZDripBackups = GetBoolArg("-backupzdrip", true);
        pwalletMain->setZDripAutoBackups(fEnableZDripBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zdripTracker->Init();
...
//    }
//
//    nAnonymizeDripAmount = GetArg("-anonymizedripamount", 0);
//    if (nAnonymizeDripAmount > 999999) nAnonymizeDripAmount = 999999;
//    if (nAnonymizeDripAmount < 2) nAnonymizeDripAmount = 2;
    ...
    LogPrintf("Anonymize DRIP Amount %d\n", nAnonymizeDripAmount);
    ...
    /* Denominations
       ...
       For example:
       1DRIP+1000 == (.1DRIP+100)*10
       10DRIP+10000 == (1DRIP+1000)*10
    */
