...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zdripTracker = unique_ptr<CzDRIPTracker>(new CzDRIPTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZDRIP_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZDRIP_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
