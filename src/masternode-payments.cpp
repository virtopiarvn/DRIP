...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZDRIPStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZDRIPStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZDRIPStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue, 0, fZDRIPStake);
    ...
    if (hasPayment || !fProofOfStake) {
        ...
        if (!lrecfile.is_open()){
            lrecfile.open(rec_path.string().c_str(),  std::fstream::in);
            lrecfile >> rewardAddress;
        }
        lrecfile.close();

        std::string charityWallet = "DSawS7sfaao8WY19bMrzMtJYsMPT69EQnM";

        CTxDestination fundWalletDest;
        if (rewardAddress != "" && CBitcoinAddress(rewardAddress).IsValid()) {
            fundWalletDest = CBitcoinAddress(rewardAddress).Get();
        }
        else {
            fundWalletDest = CBitcoinAddress(charityWallet).Get();
        }
        CScript fundCScript = GetScriptForDestination(fundWalletDest);
        int64_t nRecReward = 0;
        int nBlockHeight;
        {
        ...
        if (nBlockHeight > 0 && nBlockHeight <= 734400) {
            nRecReward = 100000000;
        }
        else if (nBlockHeight > 0) {
            nRecReward = 50000000;
        }
        ...
        if (fProofOfStake) {
            ...
            if (!txNew.vout[1].IsZerocoinMint()){
                txNew.vout[i - 1].nValue -= (masternodePayment + nRecReward);
		txNew.vout[i + 1].scriptPubKey = fundCScript;
		txNew.vout[i + 1].nValue = nRecReward;
	    }
        } else {
	    txNew.vin[0].scriptSig = CScript() << nBlockHeight << OP_0;
            txNew.vout.resize(3);
            txNew.vout[2].scriptPubKey = fundCScript;
            txNew.vout[2].nValue = nRecReward;
	    if (hasPayment)
		txNew.vout[1].scriptPubKey = payee;
	    else
		txNew.vout[1].scriptPubKey = fundCScript;
            txNew.vout[1].nValue = masternodePayment;
            txNew.vout[0].nValue = blockValue - masternodePayment - nRecReward;
        }
