...
#ifndef DRIP_ZDRIPWALLET_H
#define DRIP_ZDRIPWALLET_H
...
public:
    CzDRIPWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDRIP(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDRIP(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //DRIP_ZDRIPWALLET_H
