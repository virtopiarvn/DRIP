...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DRIP);
    ...
    if (!settings.contains("nAnonymizeDripAmount"))
        settings.setValue("nAnonymizeDripAmount", 1000);

    nAnonymizeDripAmount = settings.value("nAnonymizeDripAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDripAmount"))
        SoftSetArg("-anonymizedripamount", settings.value("nAnonymizeDripAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in drip.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDripAmount:
            return QVariant(nAnonymizeDripAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDripAmount:
            nAnonymizeDripAmount = value.toInt();
            settings.setValue("nAnonymizeDripAmount", nAnonymizeDripAmount);
            emit anonymizeDripAmountChanged(nAnonymizeDripAmount);
            break;
        case CoinControlFeatures:
