...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zdripTracker->HasMintTx(hash))) {
            //zDRIP stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZDRIP;
            ...
            sub.credit = nNet;
        } else if (isminetype mine = wallet->IsMine(wtx.vout[nIndexMN])) {
            //Masternode reward
...
//                isminetype mine = wallet->IsMine(wtx.vout[nIndexMN]);
            ...
            //Retailer reward
            CTxDestination destRT;
            int nIndexRT = wtx.vout.size() - 1;
            if (ExtractDestination(wtx.vout[nIndexRT].scriptPubKey, destRT) && IsMine(*wallet, destRT)) {
                isminetype mine = wallet->IsMine(wtx.vout[nIndexRT]);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::Generated;
                sub.address = CBitcoinAddress(destRT).ToString();
                sub.credit = wtx.vout[nIndexRT].nValue;
            }
        }
        ...
        for (const CTxOut txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zDrip;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
...
bool IsZDRIPType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZDRIP:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zDrip:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZDRIP || type == TransactionRecord::MNReward) {
