...
#include "ui_sendcoinsbtcdialog.h"
...
SendCoinsBTCDialog::SendCoinsBTCDialog(QWidget* parent) : QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
                                                    ui(new Ui::SendCoinsBTCDialog),
                                                    clientModel(0),
                                                    model(0),
...
void SendCoinsBTCDialog::setClientModel(ClientModel* clientModel)
{
...
void SendCoinsBTCDialog::setModel(WalletModel* model)
{
    this->model = model;
...
SendCoinsBTCDialog::~SendCoinsBTCDialog()
{
    QSettings settings;
...
void SendCoinsBTCDialog::on_sendButton_clicked()
{
    if (!model || !model->getOptionsModel())
        return;
    ...
    if (encStatus == model->Locked || encStatus == model->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(model->requestUnlock(AskPassphraseDialog::Context::Send_DRIP, true));
        if (!ctx.isValid()) {
...
void SendCoinsBTCDialog::send(QList<SendCoinsRecipient> recipients, QString strFee, QStringList formatted)
{
    ...
    for (int i = 0; i < formatted.size(); i++) {
        if (i >= MAX_SENDBTC_POPUP_ENTRIES) {
...
void SendCoinsBTCDialog::reject()
{
    clear();
}

void SendCoinsBTCDialog::accept()
{
    clear();
}

SendCoinsEntry* SendCoinsBTCDialog::addEntry()
{
...
void SendCoinsBTCDialog::updateTabsAndLabels()
{
    setupTabChain(0);
    coinControlUpdateLabels();
}

void SendCoinsBTCDialog::removeEntry(SendCoinsEntry* entry)
{
    entry->hide();
...
QWidget* SendCoinsBTCDialog::setupTabChain(QWidget* prev)
{
    for (int i = 0; i < ui->entries->count(); ++i) {
        SendCoinsEntry* entry = qobject_cast<SendCoinsEntry*>(ui->entries->itemAt(i)->widget());
        if (entry) {
...
void SendCoinsBTCDialog::setAddress(const QString& address)
{
...
void SendCoinsBTCDialog::pasteEntry(const SendCoinsRecipient& rv)
{
...
bool SendCoinsBTCDialog::handlePaymentRequest(const SendCoinsRecipient& rv)
{
...
void SendCoinsBTCDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, 
...
void SendCoinsBTCDialog::updateDisplayUnit()
{
...
void SendCoinsBTCDialog::updateSwiftTX()
{
...
void SendCoinsBTCDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn& sendCoinsReturn, const QString& msgArg, bool fPrepare)
{
    ...
    // This comment is specific to SendCoinsBTCDialog usage of WalletModel::SendCoinsReturn.
...
void SendCoinsBTCDialog::minimizeFeeSection(bool fMinimize)
{
...
void SendCoinsBTCDialog::on_buttonChooseFee_clicked()
{
    minimizeFeeSection(false);
}

void SendCoinsBTCDialog::on_buttonMinimizeFee_clicked()
{
    updateFeeMinimizedLabel();
    minimizeFeeSection(true);
}

void SendCoinsBTCDialog::setMinimumFee()
{
    ui->radioCustomPerKilobyte->setChecked(true);
    ui->customFee->setValue(CWallet::minTxFee.GetFeePerK());
}

void SendCoinsBTCDialog::updateFeeSectionControls()
{
...
void SendCoinsBTCDialog::updateGlobalFeeVariables()
{
...
void SendCoinsBTCDialog::updateFeeMinimizedLabel()
{
    if (!model || !model->getOptionsModel())
        return;
...
void SendCoinsBTCDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
...
void SendCoinsBTCDialog::updateSmartFeeLabel()
{
    if (!model || !model->getOptionsModel())
        return;
...
// UTXO splitter
void SendCoinsBTCDialog::splitBlockChecked(int state)
{
    if (model) {
...
//UTXO splitter
void SendCoinsBTCDialog::splitBlockLineEditChanged(const QString& text)
{
...
void SendCoinsBTCDialog::coinControlButtonClicked()
{
...
void SendCoinsBTCDialog::coinControlChangeChecked(int state)
{
...
void SendCoinsBTCDialog::coinControlChangeEdited(const QString& text)
{
...
void SendCoinsBTCDialog::coinControlUpdateLabels()
{
