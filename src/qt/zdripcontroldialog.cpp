...
#include "ui_zdripcontroldialog.h"
...
std::set<std::string> ZDripControlDialog::setSelectedMints;
...
ZDripControlDialog::~ZDripControlDialog()
{
    delete ui;
}

void ZDripControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDripControlDialog::updateList()
{
...
void ZDripControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDripControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZDrip_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDripControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZDripControlDialog::GetSelectedMints()
{
...
void ZDripControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
