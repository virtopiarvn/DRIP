...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered drip address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 DRIP.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal drip:URI (drip:XyZ...?message=example)
            {
...
static void NotifyzDRIPReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzDRIPReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzDRIPReset.connect(boost::bind(NotifyzDRIPReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzDRIPReset.disconnect(boost::bind(NotifyzDRIPReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zdripTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
