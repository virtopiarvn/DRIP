...
#if defined(NDEBUG)
#error "DRIP cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in udrip) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zDRIP is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zDRIP spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zDRIP without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zDRIP spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zDRIP spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zDRIP spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zdrip");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zDRIP mints are not already known
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
...
int64_t GetBlockValue(int nHeight)
{
    ...
    int64_t nSubsidy = 0;
    if (nHeight == 0) {
        nSubsidy = 290000 * COIN;
    } else if (nHeight <= 120 && nHeight > 0) {
        nSubsidy = 2 * COIN;
    } else if (nHeight <= 43200 && nHeight > 120) {
        nSubsidy = 11 * COIN;
    } else if (nHeight <= 129600 && nHeight > 43200) {
        nSubsidy = 151 * COIN;
    } else if (nHeight <= 216000 && nHeight > 129600) {
        nSubsidy = 131 * COIN;
    } else if (nHeight <= 302400 && nHeight > 216000) {
        nSubsidy = 111 * COIN;
    } else if (nHeight <= 388800 && nHeight > 302400) {
        nSubsidy = 91 * COIN;
    } else if (nHeight <= 475200 && nHeight > 388800) {
        nSubsidy = 56 * COIN;
    } else if (nHeight <= 561600 && nHeight > 475200) {
        nSubsidy = 46 * COIN;
    } else if (nHeight <= 648000 && nHeight > 561600) {
        nSubsidy = 26 * COIN;
    } else if (nHeight <= 734400 && nHeight > 648000) {
        nSubsidy = 11 * COIN;
    } else if (nHeight > 734400) {
        nSubsidy = 5.5 * COIN;
    } else {
        nSubsidy = 5.5 * COIN;
    }
...
CAmount GetSeeSaw(const CAmount& blockValue, int nMasternodeCount, int nHeight)
{
    ...
    ret = (ret / COIN) * COIN; // round to coin

    return ret;
}

int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZDRIPStake)
{
    if (nHeight > 0 && nHeight <= 734400) {
	blockValue -= (1 * COIN);
    }
    else {
	blockValue -= (0.5 * COIN);
	...
	ret = (ret / COIN) * COIN; // round to coin
        ...
        //When zDRIP is staked, masternode only gets 2 DRIP
        ret = blockValue / 5 * 3 * COIN;
        if (isZDRIPStake)
            ret = blockValue / 5 * 2 * COIN;
	    ret = (ret / COIN) * COIN; // round to coin
    }
...
CBitcoinAddress addressExp1("D9x5Db5yBKV1MMEZKf47Q4K7smKEM94i9k");
CBitcoinAddress addressExp2("DEBr3BAZBGVwHPzPemucJJ7QbLa4p6vRbq");
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from DRIP
...
{
    RenameThread("drip-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZDRIPMinted()
{
...
void RecalculateZDRIPSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zDRIP supply
...
bool RecalculateDRIPSupply(int nHeightStart)
{
    ...
    if (nHeightStart == Params().Zerocoin_StartHeight())
        nSupplyPrev = CAmount(29056800000000);
...
bool UpdateZDRIPSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateDRIPSupply(Params().Zerocoin_StartHeight());
    }

    //Track zDRIP money supply in the block index
    if (!UpdateZDRIPSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zDRIP supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zDripSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert DRIP to zDRIP
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zDRIP spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zDRIP serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zDRIP stake block is less than allowed start height", __func__);

    if (CZDripStake* zDRIP = dynamic_cast<CZDripStake*>(stake)) {
        CBlockIndex* pindexFrom = zDRIP->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zDRIP stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zDRIP stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zDRIP->GetValue()));
        if (nChecksum200 != zDRIP->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zDRIP->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZDRIP() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zDRIP fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zDRIP mints and %d zDRIP spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // DRIP: We use certain sporks during IBD, so check to see if they are
