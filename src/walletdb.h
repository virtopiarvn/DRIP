...
public:
    ...
    bool WriteZDRIPSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDRIPSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDRIPSeed_deprecated(uint256& seed);
    bool EraseZDRIPSeed();
    bool EraseZDRIPSeed_deprecated();

    bool WriteZDRIPCount(const uint32_t& nCount);
    bool ReadZDRIPCount(uint32_t& nCount);
