...
CzDRIPWallet::CzDRIPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdrip seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDRIPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDRIPSeed_deprecated()) {
                    LogPrintf("%s: Updated zDRIP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdrip seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdrip wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDRIPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDRIPCount(nCountLastUsed);
    else if (!walletdb.ReadZDRIPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDRIPWallet::Lock()
{
    seedMaster = 0;
}

void CzDRIPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDRIPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDRIP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDRIPWallet::LoadMintPoolFromDB()
{
...
void CzDRIPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDRIPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDRIPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdripTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDRIPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDRIP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdripTracker which also adds to database
    pwalletMain->zdripTracker->Add(dMint, true);
...
void CzDRIPWallet::SeedToZDRIP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDRIPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDRIPWallet::GenerateDeterministicZDRIP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDRIPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDRIPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
